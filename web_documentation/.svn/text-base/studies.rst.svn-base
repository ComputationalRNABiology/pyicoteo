...
Download the `exercise files <http://regulatorygenomics.upf.edu/group/media/cebpa_example.tar.gz>`_. Extract the file, and open a console and get inside the file usin the *cd* command. The file *cebpa.bed* is the result of mapping to the last human genome (hg19) to the interaction of the `CEBPA transcription factor <http://en.wikipedia.org/wiki/CEBPA>`_ to liver human cells, while the file *control.bed* is the result of immunoprecipitating "naked" DNA without a protein. This a subset (chromosome 22) of real published data from `Schmidt et Al <http://www.ncbi.nlm.nih.gov/pubmed/20378774>`_
...
This is a cross-correlation test between forward and reverse strand clusters in order to find the optimal extension length. To do this, we will analyze the experiment file, cebpa.bed::
    ...
    pyicos strcorr cebpa.bed corr.png --input-format bed --showplots
...
We will extend the bed reads to 100 and get the output as a bed file::
    ...
    pyicos extend cebpa.bed cebpa_ext100.bed 114 --experiment-format bed --output-format bed --open-output --label cebpa_ext100
    pyicos extend control.bed control_ext100.bed 114 --experiment-format bed --output-format bed --open-output --label control_ext100
...
We will cluster the reads with the *convert* command::
    ...
    pyicos convert cebpa_ext100.bed cebpa_ext100.wig --experiment-format bed --output-format bed_wig  --open-output --label 1.cebpa_ext100
    pyicos convert control.bed control_ext100.wig --experiment-format bed --output-format bed_wig  --open-output --label 2.control_ext100
...
removing reads that are not product of cebpa interaction with the genome::
    ...
    pyicos subtract cebpa_ext100.bed control_ext100.bed subtracted.wig --experiment-format bed --open-experiment --output-format bed_wig --open-output --label 3.subtracted
...
Finally, we will discard artifacts. The discard artifact algorithm will consider as an artifact any cluster that has a "block" similar shape and singletons (clusters with only one read)::
     ...
     pyicos discard subtracted.wig result.wig --experiment-format bed_wig --open-experiment --output-format bed_wig --open-output --label 4.extended_subtracted_noartifacts
